...
public :
	...
	void ScanConvertion(vec3 viewPoint, vec3 *light, vec3 *lightPosition, int lightNum, Texture *texture);		///scan covertion
	void ScanConvertion(vec3 viewPoint, vec3 *light, vec3 *lightPosition, int lightNum, Texture *texture, ModelFace *f);	//used for ScanCovertion
	void DrawLine(float a, float b, float za, float zb, vec3 na, vec3 nb, int yID, vec3 *c, vec3 *v, vec3 *l, Texture *texture);
	...
	vec3 textureCompute(vec3 *n, vec3 *p, Texture *texture);
	void PhongShading();
	void GouraudShading();
